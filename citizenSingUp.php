...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // SQL query to insert data into the citizenreg table----just here
    // Hash the password before inserting it into the database
    $hashedPassword = password_hash($password, PASSWORD_DEFAULT); //-----------------------------------------------------------hassed

    $sql = "INSERT INTO citizenreg (firstname, lastname, email, permanentaddress, secondaryaddress, city, image, country, registrationdate, gender, countrycode, phone, password) 
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        header("Location: congratulationsPHP/congratulationsctz.php");
        exit();
    } else {
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
            <div class="row jumbotron box8">
                <div class="col-sm-12 mx-t3 mb-4">
                    <h2 class="text-center text-info">Citizen Register</h2>
                </div>
